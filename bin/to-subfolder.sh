...
usage ()
{
    echo "Usage: ${0##*/} --folder|f folder --email|e email --password|p password [--org|o org-name]"
...
OPTS=$(getopt --options f:e:p:o: --longoptions folder:,email:,password:,org: --name 'parse-options' -- "$@")
...
#user="admin" &&\
#info="/projects/personal/work/bw.enc.json" &&\
#user_data=$(sops --decrypt "${info}" | jq -r ".bw.${user}") &&\
#email=$(echo "${user_data}" | jq -r '.email') &&\
#password=$(echo "${user_data}" | jq -r '.password') &&\
#dos2unix to-subfolder.sh && ./to-subfolder.sh --folder "Vitali & Kate shared" --email "${email}" --password="${password}"
